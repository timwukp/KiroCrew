...
- The audit surface is otherwise unchanged, which is what keeps this a refactor of the guarantee rather than a change to what is audited. `_audit_denied` claims the request, so the three enriched records stay single. `sel_audit_middleware` claims every request it sees, so a handler's 403 remains its business and gains no second record under a different outcome. `token_auth_middleware` RETURNS its 401/403 rather than raising and audits each with a specific reason code, and returned responses are not inspected here. Only 401/403 count as refusals — a 302 from host canonicalization and a 404 from routing pass through untouched. `test_api_health.py` drives all four properties through a real middleware chain, including a synthetic barrier that audits nothing
