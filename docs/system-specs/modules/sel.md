...
- **Key + log are on the sensitive-path floor (`cdf82704`):** the `trust` directory (whole-dir entry), the legacy `sel_hmac.key` leaf, and `security_events.jsonl` are in `security._SENSITIVE_HOME_DIRS`, so the audited agent's `fs_read`/file-edit tools (gated by `is_sensitive_path()`) cannot read the key to forge the chain or rewrite the log. The gateway's own writer/reader (`sel.py`, `dashboard/session_health.py`) opens the files directly and bypasses that gate. Residual, and it is a known gap rather than a covered one: a spawned OS subprocess reaches the key through an `open()` that never routes through `is_sensitive_path()`, and the sandbox launcher's bind-mount mask (`sandbox._KEYSTONE_LEAVES`, which covers the rest of the governance keystone) deliberately does NOT cover the trust root, because an empty inode is the wrong substitute for a signing key in both directions. Masking the `trust` DIRECTORY leaves a sandboxed Kiro Crew child (an app backend, or an MCP server under a namespace-wrapped kiro-cli on Linux) no key in the overlay: it mints a fresh one and appends to the still-visible `security_events.jsonl` with it, so `verify_integrity()` then reports an HMAC mismatch on a log nobody tampered with. Masking the pre-migration `sel_hmac.key` leaf instead hits the fail-hard length check above, turning the 0-byte overlay into a `RuntimeError` out of `SecurityEventLog()` in that child. Masking the log alongside the key would make the child's SEL self-consistent, at the price of silently dropping the audit records that child writes — a decision about how a confined child audits (the answer being a gateway-mediated append), not a mask-list entry. So the key's protection is the in-process gate plus `is_sensitive_bash_command`'s shell forms, and the OS-level backstop the other keystone leaves have is future hardening alongside the out-of-process signer: the key still lives in the agent's home namespace.
